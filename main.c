...
// »õ·Î¿î »ö»ó Á¤ÀÇ
#define THEME_BG        Black       // ¹è°æ»ö
#define THEME_HEADER    0x861D      // ÁøÇÑ º¸¶ó»ö
#define THEME_TEXT      0xBDF7      // ¹àÀº È¸»ö
#define THEME_HIGHLIGHT 0x05FF      // Ã»·Ï»ö
#define THEME_ACCENT    0xFD20      // »êÈ£»ö
#define THEME_BUTTON    0x2DB7      // Ã»³²»ö
#define THEME_WARNING   0xFBE0      // amber»ö
...
int main(void) {
  ...
  TFT_string(0, 23, Cyan, Black, "           SD Ä«µå ÃÊ±âÈ­...            ");
  Beep();
  Delay_ms(1000);
  TFT_clear_screen();
  ...
  TFT_string(0, 9, THEME_TEXT, THEME_BG, "      ÆÄÀÏ ¹øÈ£ : 000/000 (   kbps)     ");
  TFT_string(0,11, THEME_TEXT, THEME_BG, "      ÆÄÀÏ ¿ë·® : 0000KB  (     Hz)     ");
  TFT_string(0,13, THEME_TEXT, THEME_BG, "      ¿¬ÁÖ ÁøÇà : 00:00(000%)   (      )");
  TFT_string(0,15, THEME_TEXT, THEME_BG, "----------------------------------------");
  TFT_string(0,17, THEME_TEXT, THEME_BG, "   À½·®(Volume) : 000%(000/250)         ");
  TFT_string(0,19, THEME_TEXT, THEME_BG, "   ÀúÀ½(Bass)   :  00 (00 ~ 15)         ");
  TFT_string(0,21, THEME_TEXT, THEME_BG, "   °íÀ½(Treble) :  00 (-8 ~ +7)         ");
  ...
  while (1) {
    ...
    switch (key) {
      ...
      if(play_flag == 1)
          TFT_string(33, 13, THEME_HIGHLIGHT, THEME_BG, "Àç»ýÁß");
      else
          TFT_string(33, 13, THEME_TEXT, THEME_BG, " Á¤Áö ");
        break;
      ...
      // key3 ?ž…? ¥ ?‹œ ê°? ê¸°ëŠ¥?˜ ê°? ì¦ê??, ìµœë??ê°’ì— ?„?‹¬?•˜ë©? ìµœì†Œê°’ìœ¼ë¡? ?Œ?•„ê°?
      case KEY3:
          if (func_mode == 0) {            // ?‹¤?Œ ?Œ?•… ?„ ?ƒ
              if (file_number != (total_file - 1))
                  file_number++;
              else
                  file_number = 0;
              ...
              index = 512;
              VS1053b_software_reset();
          } else if (func_mode == 1) {     // ë³¼ë¥¨ ì¦ê??
              volume++;
              if (volume > 250) {
                  volume = 5;  // ìµœì†Œê°’ìœ¼ë¡? ?Œ?•„ê°?
              }
              VS1053b_SetVolume(volume);
              TFT_volume();
          } else if (func_mode == 2) {     // ë² ì´?Š¤ ì¦ê??
              bass++;
              if (bass > 15) {
                  bass = 0;  // ìµœì†Œê°’ìœ¼ë¡? ?Œ?•„ê°?
              }
              VS1053b_SetBassTreble(bass, treble);
              TFT_bass();
          } else {                         // ?Š¸? ˆë¸? ì¦ê??
      ...
      case KEY4:
          if (func_mode == 0) {            // ?´? „ ?Œ?•… ?„ ?ƒ
              ...
              // key4?— ????•œ ?‹¤ë¥? ê¸°ëŠ¥?„ ?œ„?•´ ê¸°ì¡´ ì²˜ë¦¬ë¥? ? œê±°í•˜????Šµ?‹ˆ?‹¤.
          }
          break;
...
void TFT_filename(void) {
  ...
  if (file_flag == 0)                           // Short file name (8.3 format)
    ...
    TFT_string(3, 5, Red, Black, "* ÆÄÀÏ¸í ±æÀÌ ÃÊ°ú *");
  else                                          // File name error
    TFT_string(3, 5, Red, Black, "*** ÆÄÀÏ¸í ¿À·ù ***");
