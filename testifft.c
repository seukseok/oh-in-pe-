...
#define DAC_BUFFER_SIZE SAMPLE_SIZE // DAC 버퍼 크기를 샘플 크기와 동일하게 설정

// DAC 출력 관련 변수 정의
uint16_t dac_buffer[DAC_BUFFER_SIZE];  // DAC 출력 버퍼
uint32_t dac_index = 0;                // DAC 버퍼 인덱스
...
void ProcessAndOutputAudio(complex_f *fft_data, unsigned r); // 역FFT 수행 후 DAC 출력을 위한 함수 선언
...
void TIM7_IRQHandler(void)                /* TIM7 인터럽트(25.6kHz)에서 ADC 입력 처리 */
{
  ...
  FFT_buffer[FFT_count] = ((float)ADC1->DR) / 2048.0f - 1.0f; // ADC 결과를 FFT 버퍼에 저장 및 -1.0 ~ 1.0 범위로 정규화

  // DAC 출력
  DAC->DHR12R1 = dac_buffer[dac_index];
  dac_index = (dac_index + 1) % DAC_BUFFER_SIZE;
...
int main(void)
{
  ...
  TFT_string(7,0,White,Magenta," 실시간 FFT 변환 및 표시 "); // 화면에 문자열 표시
...
void do_fft(void)
{
  ...
  for(int i = 0; i < SAMPLE_SIZE; i++)
  {
    ...
    fft_data[i].im = 0.0f;                 // 허수부를 0으로 초기화
  }
  ...
  // 오디오 출력을 위한 처리
  ProcessAndOutputAudio(fft_data, r);
}
...
/* 역FFT 수행 후 DAC 출력을 위한 함수 추가 */
void ProcessAndOutputAudio(complex_f *fft_data, unsigned r) {
    // 역FFT 수행
    ffti_f(fft_data, r, FFT_INVERSE);
    
    // DAC 출력을 위한 신호 변환
    for(int i = 0; i < SAMPLE_SIZE; i++) {
        fft_data[i].re *= 0.5f; // 출력 신호 크기 절반으로 줄여서 DAC 범위에 맞게 설정
        // 실수부만 사용하여 DAC 범위(0-4095)로 변환
        float normalized = fft_data[i].re;
        // -1~1 범위를 0~4095 범위로 변환
        dac_buffer[i] = (uint16_t)((normalized + 1.0f) * 2047.5f);
    }
}